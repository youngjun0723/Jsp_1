...
public class DBConnectionMgr2 {
    ...
    // 1. 오라클 드라이버와 URL 설정 수정
    private String _driver = "oracle.jdbc.driver.OracleDriver",
    // 1521은 오라클 기본 포트, xe는 기본 SID입니다. (환경에 따라 수정 필요)
    _url = "jdbc:oracle:thin:@localhost:1521:xe", 
    _user = "c##hr",      // 오라클 사용자 계정
    _password = "1234";  // 사용자 비밀번호
    
    private boolean _traceOn = false;
    private boolean initialized = false;
    private int _openConnections = 50;
    private static DBConnectionMgr2 instance = null;
    ...
    public static DBConnectionMgr2 getInstance() {
        if (instance == null) {
            synchronized (DBConnectionMgr2.class) {
                if (instance == null) {
                    instance = new DBConnectionMgr2();
                }
            }
        }
...
class ConnectionObject2 {
    public Connection connection = null;
    public boolean inUse = false;

    public ConnectionObject2(Connection c, boolean useFlag) {
        connection = c;
        inUse = useFlag;
    }
}
